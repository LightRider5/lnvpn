  ...
  if (props.order) {
    return (
      <div>
            ...
            <Form className="order-ID">
              <Form.Group className="mb-2">
                <InputGroup>
                  ...
                  <Form.Control value={props.order.orderId} />
                  <Button
                    size={30}
                    ref={targetID}
                    onClick={() => {
              ...
              <div>
                <Form className="order-ID">
                  <Form.Group className="mb-2">
                    <InputGroup>
                      ...
                      <Form.Control value={props.paidOrder.number} />
                      <Button
                        size={30}
                        ref={targetNumber}
                        onClick={() => {
                ...
                {props.paidOrder.code ? (
                  <Alert variant="success">
                    Received activation code: {props.paidOrder.code}
                  </Alert>
                ) : (
                  <div>
        ...
        <Alert variant="light">
          Note that if you have not received a SMS code successfully after
          20min, your payment will be canceled automatically and funds will
          return to your wallet. No refund needed!
        </Alert>

        <Overlay
          target={targetInvoice.current}
          transition={true}
          show={showTooltipInvoice}
          placement="right"
        >
          {(propsTooltip) => (
        ...
        <Overlay
          target={targetNumber.current}
          transition={true}
          show={showTooltipNumber}
          placement="bottom"
        >
          {(propsTooltip) => (
            <Tooltip id="copied-tooltip" {...propsTooltip}>
